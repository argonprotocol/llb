<template>
  <div class="Tour Component" :style="stepVars">
    ...
    <div StepWrapper class="z-[2000]" v-else>
...
const previousStep = (previousStep: number) => {
  basicStore.setConfig({ tourStep: previousStep });
  updateStepVars();
};

const nextStep = (nextStep: number) => {
  if (nextStep === 5) {
    basicStore.setConfig({ tourStep: 0, completedWelcome: true });
    return;
  }
  basicStore.setConfig({ tourStep: nextStep });
  updateStepVars();
};
