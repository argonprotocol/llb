<template>
  <Popover class="MoreInfoMenu Component px-1 cursor-pointer relative">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
      <PopoverPanel ref="menuRef" v-slot="{ close: closeFn }" class="absolute -right-9 z-[1000] mt-3 flex w-screen max-w-min px-4">
        ...
        <div class="w-64 rounded bg-white p-1 pb-1 text-sm font-light leading-6 text-gray-900 shadow-lg ring-1 ring-gray-900/5 text-right whitespace-nowrap">
          <a @click="openMoreLiquidLocking(closeFn)" class="block pt-2 pb-1 px-3 hover:text-fuchsia-600 hover:bg-[#FAF4FC]">The Details of Liquid Locking</a>
...
function openMoreLiquidLocking(closeFn?: () => void) {
